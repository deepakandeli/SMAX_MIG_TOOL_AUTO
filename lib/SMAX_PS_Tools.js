...
module.exports = {
  login2SF: function (page,newPagePromise,username, password, onCompleteSelector,timeout) {
    return loginToSalesforce(page,newPagePromise,username, password, onCompleteSelector,timeout);
  },
  ...
  initPackage : function(packageFilePath){
    return fs.readFileSync(packageFilePath, (err, data) => {
      if (err) throw err;      
    });
  },
...
async function loginToSalesforce (page,sfPagePromise,username, password, onCompleteSelector,timeout) {
  ...
  try{
    //Auth
    await Promise.all([
      sfPageMain.waitForSelector('#oaapprove',{timeout: timeout}),sfPageMain.waitForNavigation(),await Login.click()
    ]);

    //oaapprove
    sfPageMain.waitForSelector('#oaapprove',{timeout: timeout});
    const auth = await sfPageMain.$('#oaapprove');

    await Promise.all([
      page.waitForSelector(onCompleteSelector,{timeout: timeout}),await auth.click()
    ]);
  }
  catch(err){
    errorMesg="SFAuthfailed";
    throw errorMesg+' '+err.message;
  }
  return;
}
