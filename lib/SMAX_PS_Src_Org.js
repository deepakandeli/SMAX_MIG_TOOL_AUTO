...
async function setSrcOrg(browser,page,config){
    ...
    try{
        await Promise.all([
            await tools.login2SF(page,newPagePromise,config.source.Username,config.source.Password,onCompleteSelector,config.MiscSetting.TIMEOUTS.VALIDATE)
        ]);
    }
    catch(err){
        errorMesg ='Source Login failed';
        throw errorMesg+' '+err.message;
        var pathLoc = 'Sourceloginerror_'+Date.now()+'.png';
