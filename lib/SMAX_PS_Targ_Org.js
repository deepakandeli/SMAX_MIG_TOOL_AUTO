...
async function setTargOrg(browser,page,config){
    ...
    try{
        await Promise.all([
            await tools.login2SF(page,newTargPagePromise,config.target.Username,config.target.Password,'#targetLogout:not(.con-hidden)',config.MiscSetting.TIMEOUTS.VALIDATE)
        ]);
    }
    catch(err){
        errorMesg="Target Login failed";
        throw errorMesg+' '+err.message
        var pathLoc = 'Targetloginerror_'+Date.now()+'.png';
        await newTargPagePromise.screenshot({ path: pathLoc,fullPage: true});
    }    
